...
class SelectConstructor {
	...
	selectsInit(selectItems) {
		selectItems.forEach((originalSelect, index) => {
			// Проверяем, не был ли элемент уже инициализирован
			if (!originalSelect.closest('.select')) {
				this.selectInit(originalSelect, index + 1);
			}
		});

		// Обработчики событий (остаётся без изменений)
	...
	selectInit(originalSelect, index) {
		...
		// Вставляем обертку перед оригинальным <select>
		originalSelect.parentNode.insertBefore(selectItem, originalSelect);
		// Перемещаем <select> внутрь обёртки
		...
		// Вставляем тело селекта
		selectItem.insertAdjacentHTML('beforeend', `<div class="${this.selectClasses.classSelectBody}"><div hidden class="${this.selectClasses.classSelectOptions}"></div></div>`);

		// Строим псевдоселект
		this.selectBuild(originalSelect);

		// Запоминаем скорость анимации
		originalSelect.dataset.speed = originalSelect.dataset.speed ? originalSelect.dataset.speed : this.config.speed;
		this.config.speed = +originalSelect.dataset.speed;

		// Слушатель на изменение оригинального select
	...
	setSelectTitleValue(selectItem, originalSelect) {
		// Получаем тело селекта
		const selectItemBody = this.getSelectElement(selectItem, this.selectClasses.classSelectBody).selectElement;

		// Если .select__body уже существует — очищаем его
		if (selectItemBody) {
			// Очищаем всё содержимое, кроме scroll-блока (если он есть)
			const scrollEl = selectItemBody.querySelector(`.${this.selectClasses.classSelectOptions}`);
			selectItemBody.innerHTML = '';
			if (scrollEl) {
				selectItemBody.appendChild(scrollEl);
			}
		} else {
			// Если нет .select__body — создаём его
			selectItem.insertAdjacentHTML('beforeend', `<div class="${this.selectClasses.classSelectBody}"></div>`);
		}
